...
import { accountList, channelConfig, channelPayment, testQrcode } from '@/api/payment'
import { ElMessage, ElMessageBox } from 'element-plus';
import PaymentEdit   from "../form/payment-edit.vue";
...
async function fetchPaymentChannels() {
  loading.value = true;
  try {
    const res = await accountList({ app_no: viewData.appNo });
    if (res.code === 0) {
...
async function fetchChannelConfig() {
  loading.value = true;
  try {
    const res = await channelConfig({ app_no: viewData.appNo });
    if (res.code === 0) {
...
async function fetchPaymentProducts(channel) {
  loading.value = true;
  try {
    const res = await channelPayment({ channel:channel });
    if (res.code === 0) {
...
const loadApplicationQrcode = async (row) => {
  try {
    const res = await testQrcode({ app_no: row.app_no, account_no: row.account_no })
    if (res.code === 0) {
