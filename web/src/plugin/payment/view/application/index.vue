...
import {
  ...
  createApplication,
  updateApplication,
  updateApplicationStatus, getApplicationDetail, getApplicationQrcode
} from '@/plugin/payment/api/application.js'
import { ElMessage, ElMessageBox } from 'element-plus'
import ApplicationForm from "@/plugin/payment/form/application-form.vue";
import PaymentAccount  from "@/plugin/payment/components/payment-account.vue";
import { useRoute } from 'vue-router'
import {getMerchantList} from "@/plugin/payment/api/merchant.js";  // 添加路由导入
...
// 组件挂载时获取商户列表
onMounted(() => {
  const res =  getMerchantList({mch_no: route.query.mch_no, page:1, page_size:10})
  if (res.code === 0) {
...
const remoteSearchMchVenuer = async (query) => {
  ...
  try {
    if (route.query.mch_no !== '') {
      const res = await getMerchantList({mch_no: route.query.mch_no, page: 1, page_size: 10})
      if (res.code === 0) {
        venuerMchList.value = res.data.list || []
      } else {
        venuerMchList.value = []
...
const getApplication = async (appNo) => {
  try {
    const res = await getApplicationDetail(appNo)
    if (res.code === 0) {
...
const loadApplicationQrcode = async (appNo, appName) => {
  try {
    const res = await getApplicationQrcode(appNo)
    if (res.code === 0) {
      qrAppNo.value = appNo
