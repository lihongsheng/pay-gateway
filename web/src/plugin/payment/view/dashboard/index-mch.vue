...
import {
  getDashboardMchAppData,
  getDashboardMchData, getDashboardTotalRequest,
  searchDashboard
} from '@/plugin/payment/api/dashboard.js'
import { getApplicationList } from "@/plugin/payment/api/application.js"
import { getAccountList } from "@/plugin/payment/api/payment_account.js"
...
const fetchAppOptions = async () => {
  try {
    const res = await getApplicationList({ page: 1, page_size: 100 })
    if (res.code === 0) {
...
const searchAppData = async () => {
  ...
  try {
    ...
    const res = await getDashboardMchAppData(params)
    if (res.code === 0) {
...
const fetchSystemData = async () => {
  try {
    ...
    const res = await getDashboardMchData(params)
    const totalRequestRes = await getDashboardTotalRequest(params)
    if (res.code === 0) {
