...
func AggregateRedirectUrl(c *gin.Context) {
...
func AggregateTestRedirectUrl(c *gin.Context) {
	var req public.AggregateRedirectUrlRequest
	if err := c.ShouldBindQuery(&req); err != nil {
		FailMessage(errors.ErrCodeInvalidParam, err.Error(), c)
		return
...
func AggregateTestPayment(c *gin.Context) {
...
func AggregateGetUserOpenID(c *gin.Context) {
...
func AggregateGetApplication(c *gin.Context) {
	token := c.Query("token")
	if token == "" {
		FailMessage(errors.ErrCodeInvalidParam, "token is empty", c)
		return
	...
	response.OK(c, public.AggregateApplication{
		AppNo:        r.AppNo,
...
func AggregatePayment(c *gin.Context) {
	var req public.AggregateOrder
	if err := c.ShouldBindJSON(&req); err != nil {
		FailMessage(errors.ErrCodeInvalidParam, err.Error(), c)
		return
...
func AggregateQuery(c *gin.Context) {
	token := c.Query("token")
