...
// ssh root@8.136.12.80
// T-CHnu9?qr8V]grDp4aN
...
func NewAggregateService(svc *svc.ServiceContext, domain *domain.ServiceGroup) Aggregate {
...
func (a *aggregateService) GenTestQrCode(ctx context.Context, appNo string, accNo string) (qrLink string, orderNo string, err error) {
	l := log.WithCtx(ctx)
	appInfo, err := a.svc.AppRepo.GetByAppNoFormCache(ctx, appNo)
	if err != nil {
...
func (a *aggregateService) GetApplication(ctx context.Context, token string) (*model.Application, error) {
	...
	app, err := a.svc.AppRepo.GetByAppNoFormCache(ctx, aggregateToken.AppNo)
	if err != nil {
...
func (a *aggregateService) GetUserOpenID(ctx context.Context, req public.AggregateUserOpenIDRequest) (string, error) {
	...
	accountDetail, err := a.svc.PaymentAccountRepo.GetByAccountNoFormCache(ctx, req.AccountNo)
	if err != nil {
	...
	app, err := a.svc.AppRepo.GetByAppNoFormCache(ctx, accountDetail.AppNo)
	if err != nil {
		l.Error("GetApplication", zap.Error(err), zap.String("app_no", accountDetail.AppNo), zap.Any("req", req))
		return "", errors2.NewError(errors2.ErrCodeInvalidParam, "无效应用")
	}
	agg, ok := a.svc.AggregateAdapter[channel.Channel(channel.Channel_value[accountDetail.Channel])]
	if ok {
		openid, err := agg.GetUserOpenID(ctx, req.AuthCode, accountDetail, app, req.PaymentMethod)
		if err != nil {
			if errors2.IsUserLimitError(err) {
				e := a.svc.Event.Publish(ctx, &event.UserLoginLimitEvent{
					AppNo:     app.AppNo,
					UserLimit: true,
					AccountNo: req.AccountNo,
					CreateAt:  time.Now(),
				}, config.Config.Topic.UserLimit)
				if e != nil {
...
func (a *aggregateService) GetRedirectUrl(ctx context.Context, req *public.AggregateRedirectUrlRequest, basePath string) (string, error) {
	...
	if global.GVA_CONFIG.Env.IsTest() && req.FilterAccountNo == "" && req.OrderNo == "T1772159917" {
		req.FilterAccountNo = "P4966c7e946c00,P6b5e44d8e7c00"
	}
	if global.GVA_CONFIG.Env.IsTest() && req.FilterAccountNo == "" && req.OrderNo == "T17721599111" {
	...
	appInfo, err := a.svc.AppRepo.GetByAppNoFormCache(ctx, aggregateToken.AppNo)
	if err != nil {
		return "", errors2.NewError(errors2.ErrCodeInvalidParam, "token无效")
	}
...
func (a *aggregateService) getAccount(ctx context.Context, req *public.AggregateRedirectUrlRequest, app *model.Application, l *zap.Logger) (*entity.PaymentAccount, error) {
	if req.AccountNo != "" {
		account, err := a.svc.PaymentAccountRepo.GetByAccountNoFormCache(ctx, req.AccountNo)
		if err != nil {
	...
	available, err := a.domain.Router.GetAvailablePayment(ctx, &public.QueryCheckoutPaymentMethod{
	...
	available, lastAvailable, err = a.domain.Router.Router(ctx, available, app, filterAccountNo)
	if err != nil {
	...
	account, _ := a.domain.Router.Rand(available)
...
func (a *aggregateService) Payment(ctx context.Context, req *public.AggregateOrder, notify string) (*public.PaymentOrderResponse, error) {
	...
	r, _, err := a.domain.PaymentService.Payment(ctx, nil, order, appInfo, account, callbackUrl)
	if err != nil {
		l.Error("AggregatePayment", zap.Error(err), zap.Any("req", req))
		return nil, err
	}
...
func (a *aggregateService) Query(ctx context.Context, token string, orderNo string) (*public.PaymentOrderDetail, error) {
	...
	order, err := a.domain.PaymentService.Query(ctx, &public.QueryPaymentRequest{
		OrderNo: orderNo,
		AppNo:   appInfo.AppNo,
		TradeNo: "",
		MchNo:   appInfo.MchNo,
...
func getAggregateToken(token string) (*dto.AggregateToken, error) {
	...
	str, err := utils.AesGCMDecryptHex(token, []byte(config.Config.Salt))
	if err != nil {
		return nil, errors2.NewError(errors2.ErrCodeInvalidParam, "token无效")
	}
...
func genAggregateToken(req dto.AggregateToken) (string, error) {
	...
	str, err := utils.AesGCMEncryptHex(by, []byte(config.Config.Salt))
	if err != nil {
		return "", errors2.NewError(errors2.ErrCodeInvalidParam, "token无效")
	}
	return str, nil
...
func getWebBaseUrl(app *model.Application) (*url.URL, error) {
	...
	if app.IsCustomerDomain == enum.CustomerDomainStatus_Enable {
		...
		u = u.JoinPath(config.Config.ProxyNotifyPrefix)
	}
	return u, nil
}
