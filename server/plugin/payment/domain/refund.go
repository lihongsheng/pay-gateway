...
func (s *refundService) Refund(ctx context.Context, req *public.RefundRequest, appInfo *model.Application) (m *model.RefundOrder, err error) {
	...
	l := log.WithCtx(ctx)
	lock, err := global.Redis.SetNX(ctx, s.getRefundLockKey(req.OrderNo, req.AppNo), time.Now().Unix(), enum.PaymentLockExpire).Result()
	defer func() {
		global.Redis.Del(ctx, s.getRefundLockKey(req.OrderNo, req.AppNo))
	}()
	if !lock && err == nil {
