...
import (
	...
	systemReq "github.com/lihongsheng/pay-gateway/model/system/request"
...
func (userService *UserService) GetUserInfoList(info systemReq.GetUserList) (list interface{}, total int64, err error) {
	...
	users, err := userService.formatUser(userList)
	if err != nil {
		return
	}
...
type SysUser struct {
	...
	Authorities   []system.SysAuthority `json:"authorities" gorm:"many2many:sys_user_authority;"`                                                   // 多用户角色
	...
	MchNo         string                `json:"mchNo" gorm:"column:mch_no;default:0;comment:商户ID"`
	MchName       string                `json:"mchName"`
}

func (userService *UserService) formatUser(userList []system.SysUser) ([]SysUser, error) {
	var list = make([]SysUser, 0, len(userList))
	ids := make([]string, 0, len(userList))
	for _, k := range userList {
		list = append(list, sysUserFormat(k))
		if k.UserType.ISMch() && k.MchNo != "" {
			ids = append(ids, k.MchNo)
		}
	}
	ctx := context.Background()
	mch, err := svc.ServiceContextApp.MchRepo.Search(ctx, admin.MchQueryRequest{MchNos: ids, Page: 1, PageSize: len(ids)})
	if err != nil {
		return nil, err
	}
	for i := range list {
		for ii := range mch {
			if mch[ii].MchNo == list[i].MchNo {
				list[i].MchName = mch[ii].MchName
			}
		}
	}
	return list, nil
...
func sysUserFormat(k system.SysUser) SysUser {
	return SysUser{
		GVA_MODEL:     k.GVA_MODEL,
		UUID:          k.UUID,
		Username:      k.Username,
		Password:      k.Password,
		NickName:      k.NickName,
		HeaderImg:     k.HeaderImg,
		AuthorityId:   k.AuthorityId,
		Authority:     k.Authority,
		Authorities:   k.Authorities,
		Phone:         k.Phone,
		Email:         k.Email,
		Enable:        k.Enable,
		OriginSetting: k.OriginSetting,
		UserType:      k.UserType,
		MchNo:         k.MchNo,
	}
}
...
func (userService *UserService) SetUserInfo(req system.SysUser) error {
	ups := map[string]interface{}{
		...
		"email":      req.Email,
		"enable":     req.Enable,
		"user_type":  enum.UserType_Platform,
		"mch_no":     req.MchNo,
	}
	if req.MchNo != "" {
		ups["user_type"] = enum.UserType_Mch
	}
...
func (userService *UserService) GetUserInfo(uuid uuid.UUID) (user SysUser, err error) {
	...
	user = sysUserFormat(reqUser)
	if user.UserType.ISMch() {
		mch, err := svc.ServiceContextApp.MchRepo.GetByMchNo(context.Background(), user.MchNo)
		if err != nil {
			return SysUser{}, err
		}
		user.MchName = mch.MchName
	}
	return user, err
}
