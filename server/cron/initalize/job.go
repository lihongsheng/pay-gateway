	...
	return []cron.Job{
		{
			...
			JobName:     "删除时间记录",
			Job:         job1.NewJob().DeleteEventRecord,
			MaxExecTime: 2 * time.Minute,
		},
		{
			Spec:        "0 10 2 * * 1", // 每周一凌晨2点执行
			JobName:     "删除请求日志",
			Job:         job1.NewJob().DeletePaymentRequestLog,
			MaxExecTime: 2 * time.Minute,
		},
		{
			Spec:        "0 15 2 * * *", // 每天凌晨2点执行
			JobName:     "删除通知日志",
			Job:         job1.NewJob().DeleteNotifyRecord,
			MaxExecTime: 2 * time.Minute,
		},
		// 每五秒执行一次
		{
			Spec:        "@every 5s",
			JobName:     "通知三方支付订单",
			Job:         job1.NewJob().CronPaymentPushNotifyHandler,
		...
		// 每五秒执行一次
		{
			Spec:        "@every 5s",
			JobName:     "通知三方退款订单",
			Job:         job1.NewJob().CronRefundPushNotifyHandler,
			MaxExecTime: time.Hour,
		},
		{
			Spec:        "0 0 1 * * *", // 每天凌晨1点执行
		...
		},
		{
			...
			JobName:     "删除订单过期记录",
			Job:         job1.NewJob().DeleteExpireRecord,
			MaxExecTime: 2 * time.Minute,
		},
		{
			Spec:        "@every 10m", // 每10分钟执行一次
			JobName:     "支付订单过期处理",
			Job:         job1.NewJob().PaymentExpireRecordJob,
			MaxExecTime: time.Hour,
		},
		{
			Spec:        "0 30 2 1 * *", // 每月凌晨2点执行
			JobName:     "支付订单过期处理",
			Job:         job1.NewJob().DeleteExpireRouterRecord,
			MaxExecTime: time.Hour,
		},
	}
}
